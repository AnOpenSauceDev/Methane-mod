...
import me.wolfie.methane.backportutils.RotationAxis;
...
public abstract class GameRendererMixin {
    ...
    public void renderWorld(float tickDelta, long limitTime, MatrixStack matrices) {
        ...
        if (g > 0.0F) {
            ...
            matrixStack.multiply(RotationAxis.toOldMath(rotationAxis.rotationDegrees(((float)this.ticks + tickDelta) * (float)i)));
            matrixStack.scale(1.0F / h, 1.0F, 1.0F);
            float j = -((float)this.ticks + tickDelta) * (float)i;
            matrixStack.multiply(RotationAxis.toOldMath(rotationAxis.rotationDegrees(j)));
        }
        ...
        matrices.multiply(RotationAxis.toOldMath(RotationAxis.POSITIVE_X.rotationDegrees(camera.getPitch())));
        matrices.multiply(RotationAxis.toOldMath(RotationAxis.POSITIVE_Y.rotationDegrees(camera.getYaw() + 180.0F)));
    ...
    void RenderWorldVanilla(MatrixStack matrices, float tickDelta, long limitTime){
        ...
        matrices.multiply(RotationAxis.toOldMath(RotationAxis.POSITIVE_Y.rotationDegrees(camera.getYaw() + 180.0f)));
        Matrix3f matrix3f = new Matrix3f(matrices.peek().getNormalMatrix());
        matrix3f.invert(); // split into 2 lines
        RenderSystem.setInverseViewRotationMatrix(matrix3f);
